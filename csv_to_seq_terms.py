...
def construct_episodes(observation_data, action_data):
    ...
    for obs, act in zip(observation_data, action_data):
        ...
        if timestep == 0 and not first_episode:
            ...
            current_episode["terminals"] = np.array(current_episode["terminals"])
        ...
        current_episode["terminals"].append( np.array(False) )
    ...
    for t in range(T):
        action = th.Tensor( [float(i) for i in current_episode["actions"][t]] )
        obs, reward, done_, term, _ = env.step(action)
        current_episode["rewards"].append(reward-prev_reward)
        prev_reward = reward
...
with open("./decision-transformer/gym/data/blend-medium-v3.pkl", "wb") as f:
