...
class BlendEnv_Omni(CMDP):
    ...
    def __init__(self, env_id, **kwargs):
        ...
        self.M = 100            # Negative reward (penalty) constant factor for breaking in/out rule
        self.Q = 0              # Negative reward (penalty) constant factor for breaking concentrations reqs
        self.P = 2              # Negative reward (penalty) constant factor for breaking tank bounds reqs
        self.B = 2              # Negative reward (penalty) constant factor for breaking buy/sell bounds reqs
        self.Z = 10             # Positive reward multiplier to emphasize that "selling is good"
        self.D = 0.1            # Multiplier representing the influence of the depth
        self.L0_pen = 1
        ...
        with open(f"./configs/json/connections_{self.layout}.json" ,"r") as f:
            connections_s = f.readline()
        self.connections = json.loads(connections_s)
    
        with open(f"./configs/json/action_sample_{self.layout}.json" ,"r") as f:
            action = f.readline()
    ...
    def step(self, action: th.Tensor):
        ...
        for s in self.sources:
            ...
            if newinv > self.s_inv_ub[s] + self.eps: # inv too high -> reduce bought amount
                ...
                self.env_spec_log["B"] -= self.B * (self.L0_pen + newinv - self.s_inv_ub[s])
                self.env_spec_log["n_B"] += 1
                ...
                self.env_spec_log["B"] -= self.B * (self.L0_pen + self.s_inv_lb[s] - newinv)
                self.env_spec_log["n_B"] += 1
            ...
            reward += incr
            self.env_spec_log["rew_depth"] += incr
        ...
        for j in self.blenders:
            ...
            if (in_flow_sources + in_flow_blend > self.eps) and (out_flow_blend + out_flow_demands > self.eps):
                ...
                cost += self.M
                self.env_spec_log["M"] -= self.M
                self.env_spec_log["n_M"] += 1
            ...
            if newinv > self.b_inv_ub[j] + self.eps: # inv too high -> reduce incoming amount
                ...
                self.env_spec_log["P"] -= self.P * (self.L0_pen + newinv - self.b_inv_ub[j])
                self.env_spec_log["n_P"] += 1
                ...
                self.env_spec_log["P"] -= self.P * (self.L0_pen + self.b_inv_lb[j] - newinv)
                self.env_spec_log["n_P"] += 1
        ...
        for p in self.demands:
            ...
            if newinv > self.d_inv_ub[p] + self.eps: # inv too high -> reduce incoming amount
                ...
                self.env_spec_log["B"] -= self.B * (self.L0_pen + newinv - self.d_inv_ub[p])
                ...
                self.env_spec_log["B"] -= self.B * (self.L0_pen + self.d_inv_lb[p] - newinv)
        ...
        n_violations = self.env_spec_log["n_P"] if self.P > 0 else 0 + \
                        self.env_spec_log["n_B"] if self.B > 0 else 0 + \
                        self.env_spec_log["n_M"] if self.M > 0 else 0 + \
                        self.env_spec_log["n_Q"] if self.Q > 0 else 0
        ...
        return th.tensor(self.flatt_state), th.tensor(reward), th.tensor(cost), th.tensor(self.terminated), th.tensor(self.truncated), \
                    {"dict_state": self.state, "pen_tracker": self.env_spec_log, "terminated": self.terminated, "truncated": self.truncated}
    
        
    def reset(self, seed = 0, options = None):
        self.t = self.reward_ep = self.cost_ep = 0
        self.get_new_start_state()
        self.truncated = self.terminated = False
        self.env_spec_log = {"M": 0, "B": 0, "P": 0, "Q": 0, 
                            "n_M": 0, "n_B": 0, "n_P": 0, "n_Q": 0, 
                            "units_sold": 0, "units_bought": 0, "rew_sold": 0, "rew_depth": 0, "rew_bought": 0}
        self.flatt_state, _ = flatten_and_track_mappings(self.state)
        return th.Tensor(self.flatt_state), {"dict_state": self.state, "pen_tracker": self.env_spec_log, "terminated": self.terminated, "truncated": self.truncated}
    ...
    def update_reward2(self, action):
        ...
        self.env_spec_log["rew_sold"] += reward
        for s in self.sources:
        ...
        self.env_spec_log["units_sold"] += units_sold
        self.env_spec_log["units_bought"] += units_bought
    ...
    def penalty_quality(self, p, q, j, action):
        if (self.state['properties'][j][q] < self.sigma_lb[p][q] - self.eps or self.state['properties'][j][q] > self.sigma_ub[p][q] + self.eps) \
            ...
            self.env_spec_log["n_Q"] += 1
            self.env_spec_log["Q"] -= self.Q
            return self.Q
        return 0
    ...
    def penalize_action_preflows(self, action):
        ...
        for s in self.sources:
            if action["tau"][s] > self.state["sources_avail_next_0"][s]:
                ...
                self.env_spec_log["B"] -= self.B * (self.L0_pen + action["tau"][s] - self.state["sources_avail_next_0"][s])
        ...
        for p in self.demands:
            if action["delta"][p] > self.state["demands_avail_next_0"][p]:
                ...
                self.env_spec_log["B"] -= self.B * (self.L0_pen + action["delta"][p] - self.state["demands_avail_next_0"][p])
